...
export default class Scroller {
  constructor(game, clickObject, maskLimits = {}, options = {}) {
    ...
    this.tweenScroll = this.game.add
      .tween(this.scrollObject)
      .to({}, 0, Phaser.Easing.Quartic.Out);
  ...
  handleUp(target, pointer) {
    ...
    if (!this.o.infinite && this.scrollObject[this.o.direction] > this.max) {
      ...
      !this.o.infinite &&
      this.scrollObject[this.o.direction] < this.min
    ) {
  ...
  _addMomentum(o) {
    ...
    offset =
      this.diff > 0 ? -this.o.multiplier * offset : this.o.multiplier * offset;
  ...
  _addSwiping(o, pointer) {
    ...
    if (
      ...
      let direction =
        pointer[this.o.direction] < this.down ? 'forward' : 'backward';
  ...
  handleUpdate() {
    if (!this.enabled) return;
    if (this.o.infinite) {
      this.dispatchValues.total = Phaser.Math.wrap(
    ...
    this.dispatchValues.percent = MathUtils.percentageBetween2(
